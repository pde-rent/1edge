...
export class OHLCStorageService {
  ...
  async runDataSanityCheck(symbols: Symbol[]): Promise<void> {
    ...
    for (const symbol of symbols) {
      ...
      for (const timeframe of this.STORED_TIMEFRAMES) {
        try {
          const existingCandles = await this.getCandles(symbol, timeframe, minRequiredTime);
          
          if (existingCandles.length === 0 || existingCandles[0].timestamp > minRequiredTime) {
            logger.info(`Missing historical data for ${pair}:${timeframe}s, filling gaps...`);
            await this.fillHistoricalData(symbol, timeframe);
          } else {
            logger.debug(`Historical data OK for ${pair}:${timeframe}s`);
          }
        } catch (error) {
          logger.error(`Failed to check data for ${pair}:${timeframe}s:`, error);
        }
      }
    }
  ...
  private async fillHistoricalData(symbol: Symbol, timeframe: OHLCTimeframe): Promise<void> {
    ...
    try {
      ...
      const ohlcvData = await this.ccxtExchange.fetchOHLCV(
        pair,
        ccxtTimeframe,
        startTime,
        undefined, // No limit, get all available data
        { startTime }
      );
      ...
      // If timeframe is M1, also construct M5 and M30 data
