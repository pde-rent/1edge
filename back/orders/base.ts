...
export abstract class BaseOrderWatcher implements OrderWatcher {
   ...
   */
  protected calculateStepAmount(totalAmount: string, steps: number, currentStep: number): string {
...
export abstract class TimeBasedOrderWatcher extends BaseOrderWatcher {
  protected checkTimeInterval(lastTriggerTime: number, intervalMs: number): boolean {
    const now = Date.now();
    return now >= lastTriggerTime + intervalMs;
  }

  protected getNextTriggerTime(lastTriggerTime: number, intervalMs: number): number {
...
export abstract class PriceBasedOrderWatcher extends BaseOrderWatcher {
  ...
  protected calculatePriceWithSlippage(price: number, slippagePct: number = 0.5): number {
...
export abstract class SteppedOrderWatcher extends BaseOrderWatcher {
  ...
  protected calculateStepProgress(currentStep: number, totalSteps: number): number {
   ...
   */
  protected getTriggerAmount(order: Order, totalAmount: string, totalSteps: number): string {
