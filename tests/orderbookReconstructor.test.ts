...
describe('OrderbookReconstructor Mathematics', () => {
  ...
  beforeAll(async () => {
    ...
    await initStorage({ dbPath: './test-data/test.db' });
  ...
  describe('Decimal Scaling Issues', () => {
    ...
    it('should detect exponential number issues', () => {
      ...
      testValues.forEach(value => {
        ...
        console.log(`Value: ${value} -> Parsed: ${parsed} -> Exponential: ${isExponential}, Huge: ${isHuge}, Tiny: ${isTiny}`);
  ...
  describe('Order Processing Logic', () => {
    it('should correctly separate bids and asks', () => {
      ...
      const orders: OneInchOrder[] = [
        ...
        createMockOrder(ETH_ADDRESS, USDT_ADDRESS, '3000', '0.000333', '1000000000000000000', '3000000000'),
        // Bid: USDT (maker) for WETH (taker) - buying ETH with USDT  
        createMockOrder(USDT_ADDRESS, ETH_ADDRESS, '3000000000', '0.000000000333', '3000000000', '1000000000000000000'),
      ];
      ...
      console.log(`Separated - Bids: ${result.bidOrders.length}, Asks: ${result.askOrders.length}`);
      
      result.askOrders.forEach((order, i) => {
        console.log(`Ask ${i}: makerRate=${order.makerRate}, takerRate=${order.takerRate}`);
      });
      
      result.bidOrders.forEach((order, i) => {
        console.log(`Bid ${i}: makerRate=${order.makerRate}, takerRate=${order.takerRate}`);
      });
    ...
    it('should correctly process orders to levels', () => {
      ...
      const bidOrders: OneInchOrder[] = [
        createMockOrder(USDT_ADDRESS, ETH_ADDRESS, '2900000000', '0.000000000345', '2900000000', '1000000000000000000'),
        createMockOrder(USDT_ADDRESS, ETH_ADDRESS, '2950000000', '0.000000000339', '2950000000', '1000000000000000000'),
      ];
      ...
      bidLevels.forEach((level, i) => {
        console.log(`  Level ${i}: price=${level.price}, amount=${level.amount}, total=${level.total}, count=${level.count}`);
    ...
    it('should test rate and amount scaling formulas', () => {
      ...
      console.log(`  Maker Amount: ${rawMakerAmount} (${parseFloat(rawMakerAmount) / Math.pow(10, baseDecimals)} WETH)`);
      ...
      const scaledAmount = parseFloat(rawMakerAmount) / Math.pow(10, baseDecimals);
    ...
    it('should debug actual API data structure', async () => {
      ...
      try {
        ...
        const result = await reconstructor.reconstructOrderbook(1, ETH_ADDRESS, USDT_ADDRESS, 10);
        ...
        result.bids.slice(0, 3).forEach((level, i) => {
          ...
          if (price > 1e15) {
            console.log(`    ERROR: Bid price is exponentially large: ${price}`);
          }
        });
        ...
        result.asks.slice(0, 3).forEach((level, i) => {
          ...
          if (price > 1e15) {
            console.log(`    ERROR: Ask price is exponentially large: ${price}`);
          }
        });
    ...
    it('should test fixed decimal handling with WETH/USDT', async () => {
      ...
      try {
        ...
        const result = await reconstructor.reconstructOrderbook(1, ETH_ADDRESS, USDT_ADDRESS, 20);
        ...
        if (bestBidPrice > 0 && bestAskPrice > 0 && bestBidPrice < 1e10 && bestAskPrice < 1e10) {
          ...
          if (spreadPercent > 10) {
            console.log(`  WARNING: Spread seems too large (${spreadPercent.toFixed(2)}%)`);
          } else if (spreadPercent < 0.01) {
            console.log(`  WARNING: Spread seems too small (${spreadPercent.toFixed(4)}%)`);
          } else {
