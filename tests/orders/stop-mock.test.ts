...
describe("Stop Limit Order Lifecycle Test", () => {
  ...
  test("Stop Limit order triggers when price reaches stop price", async () => {
    ...
    const order = await OrderFactory.stop(
      context.testWallet,
      stopLimitParams,
      INITIAL_ETH_PRICE,
    );
    ...
    console.log(
      `Initial order trigger count: ${updatedOrder!.triggerCount}, status: ${updatedOrder!.status}, current price: ${INITIAL_ETH_PRICE}, stop price: ${STOP_PRICE}`,
    );
