...
describe("Historical Data Retrieval", () => {
  ...
  it("should automatically retrieve historical data for all configured symbols on startup", async () => {
    ...
    for (const [aggregateSymbol, tickerConfig] of Object.entries(
      tickerConfigs,
    )) {
    ...
    console.log(
      `✅ Found ${binanceSymbols.length} Binance symbols in configuration`,
    );
    ...
    // Get 1-minute data
    const candlesM1 = await storage.getCandles(
      testSymbol,
      60,
      sevenDaysAgo,
      now,
    ...
    console.log(
      `   Date range: ${new Date(firstCandle.timestamp).toISOString()} to ${new Date(candlesM1[candlesM1.length - 1].timestamp).toISOString()}`,
    );
  ...
  it("should automatically construct higher timeframes from 1-minute data", async () => {
    ...
    const candlesM5 = await storage.getCandles(
      testSymbol,
      300,
      sevenDaysAgo,
      now,
    ...
    const candlesM30 = await storage.getCandles(
      testSymbol,
      1800,
      sevenDaysAgo,
      now,
    ...
    console.log(
      `✅ Higher timeframes constructed: 5m: ${candlesM5.length}, 30m: ${candlesM30.length} candles`,
    );
  }, 15000);
  ...
  it("should verify data completeness and statistics", async () => {
    ...
    console.log(
      `   1m: ${oneMinStats.count} candles (≈${(oneMinStats.count / 1440).toFixed(1)} days)`,
    );
