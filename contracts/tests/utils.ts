...
export function createTable(
  ...
  let col1Width = Math.min(maxWidth, Math.max(...rows.map(([key]) => key.length), "Parameter".length));
  let col2Width = Math.min(maxWidth, Math.max(...rows.map(([, value]) => value.length), "Value".length));
  ...
  lines.push(`│ ${"Parameter".padEnd(col1Width)} │ ${"Value".padEnd(col2Width)} │`);
  lines.push(headerSeparator);

  for (const [key, value] of rows) {
    // Truncate with ellipsis if content exceeds maxWidth
    const truncatedKey = key.length > col1Width ? key.slice(0, col1Width - 3) + "..." : key;
    const truncatedValue = value.length > col2Width ? value.slice(0, col2Width - 3) + "..." : value;
...
function _parseSalt(salt: string): { senderBytes: SenderBytes; redeployProtectionFlag: RedeployProtectionFlag } {
  ...
  const senderBytes = isZeroAddress ? SenderBytes.ZeroAddress : SenderBytes.ZeroAddress; // Default to ZeroAddress for hash salts
...
export function guardSalt(salt: string, msgSender: string, chainId: number): string {
  const { senderBytes, redeployProtectionFlag } = _parseSalt(salt);
  
  console.log(`🔍 Salt parsing - SenderBytes: ${senderBytes}, RedeployProtectionFlag: ${redeployProtectionFlag}`);
  ...
  if (senderBytes === SenderBytes.MsgSender && redeployProtectionFlag === RedeployProtectionFlag.True) {
    ...
    return ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(["address", "uint256", "bytes32"], [msgSender, chainId, salt]));
    ...
    throw new Error("InvalidSalt: 21st byte must be 0x00 or 0x01 for MsgSender");
    ...
    return (salt !== generatedSalt) ? ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(["bytes32"], [salt])) : salt;
  }
}
