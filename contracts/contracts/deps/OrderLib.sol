...
library OrderLib {
    ...
    bytes32 internal constant _LIMIT_ORDER_TYPEHASH = keccak256(
        "Order(" "uint256 salt," "address maker," "address receiver," "address makerAsset," "address takerAsset,"
        "uint256 makingAmount," "uint256 takingAmount," "uint256 makerTraits" ")"
    );
    ...
    function calculateMakingAmount(
        ...
        return IAmountGetter(address(bytes20(data))).getMakingAmount(
            order, extension, orderHash, msg.sender, requestedTakingAmount, remainingMakingAmount, data[20:]
        );
    }
    ...
    function calculateTakingAmount(
        ...
        return IAmountGetter(address(bytes20(data))).getTakingAmount(
            order, extension, orderHash, msg.sender, requestedMakingAmount, remainingMakingAmount, data[20:]
        );
    }
     ...
     */
    function isValidExtension(IOrderMixin.Order calldata order, bytes calldata extension)
        internal
        pure
        returns (bool, bytes4)
    {
