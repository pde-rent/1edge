...
import {
  ...
  getOHLCStorage,
  OHLCTimeframe,
} from "../back/services/ohlcStorage";
...
class OHLCTester {
  ...
  private async testAPIEndpoints(): Promise<void> {
    await this.runTest("API Endpoints", async () => {
      ...
      if (!ohlcResponse.ok) {
        throw new Error(
          `OHLC API returned ${ohlcResponse.status}: ${await ohlcResponse.text()}`,
        );
      }
      ...
      if (!statsResponse.ok) {
        throw new Error(
          `Stats API returned ${statsResponse.status}: ${await statsResponse.text()}`,
        );
      }
      ...
      logger.info(
        "Statistics:",
        JSON.stringify(statsData.data.statistics, null, 2),
      );
    });
  }
  ...
  private async runTest(
    ...
    try {
      ...
      logger.error(
        `❌ ${name} - FAILED: ${error.message} (${Date.now() - startTime}ms)`,
      );
    }
  }
