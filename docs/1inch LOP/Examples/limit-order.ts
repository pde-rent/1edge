...
describe('📝 Limit Order Protocol Integration', function () {
  ...
  it('🎯 Execute basic limit order (DAI → WETH)', async function () {
    const { tokens, contracts, chainId } = await loadFixture(deployAndInitialize);
    ...
    expect(await dai.balanceOf(maker.address))
      .to.equal(balancesBefore.makerDai - orderConfig.makingAmount);
    expect(await dai.balanceOf(taker.address))
      .to.equal(balancesBefore.takerDai + orderConfig.makingAmount);
    expect(await weth.balanceOf(maker.address))
      .to.equal(balancesBefore.makerWeth + orderConfig.takingAmount);
    expect(await weth.balanceOf(taker.address))
      .to.equal(balancesBefore.takerWeth - orderConfig.takingAmount);
  });
  ...
  it('🔄 Execute partial limit order fill', async function () {
    ...
    expect(await dai.balanceOf(maker.address))
      .to.equal(balancesBefore.makerDai - partialFillAmount);
    expect(await dai.balanceOf(taker.address))
      .to.equal(balancesBefore.takerDai + partialFillAmount);
    expect(await weth.balanceOf(maker.address))
      .to.equal(balancesBefore.makerWeth + expectedTakingAmount);
    expect(await weth.balanceOf(taker.address))
      .to.equal(balancesBefore.takerWeth - expectedTakingAmount);
  });
  ...
  it('🔁 Execute multiple fills of the same order', async function () {
    ...
    expect(balancesAfterFirst.makerDai)
      .to.equal(initialBalances.makerDai - firstFillAmount);
    expect(balancesAfterFirst.makerWeth)
      .to.equal(initialBalances.makerWeth + firstExpectedTaking);
    ...
    expect(finalBalances.makerDai)
      .to.equal(initialBalances.makerDai - totalMakingFilled);
    expect(finalBalances.takerDai)
      .to.equal(initialBalances.takerDai + totalMakingFilled);
    expect(finalBalances.makerWeth)
      .to.equal(initialBalances.makerWeth + totalTakingFilled);
    expect(finalBalances.takerWeth)
      .to.equal(initialBalances.takerWeth - totalTakingFilled);
  });
  ...
  it('🔒 Execute private order with restricted taker', async function () {
    ...
    await swap.connect(resolver).fillOrderArgs(
      order,
      r,
      vs,
      orderConfig.makingAmount,
    ...
    expect(await dai.balanceOf(maker.address))
      .to.equal(balancesBefore.makerDai - orderConfig.makingAmount);
    expect(await dai.balanceOf(resolver.address))
      .to.equal(balancesBefore.resolverDai + orderConfig.makingAmount);
    expect(await weth.balanceOf(maker.address))
      .to.equal(balancesBefore.makerWeth + orderConfig.takingAmount);
    expect(await weth.balanceOf(resolver.address))
      .to.equal(balancesBefore.resolverWeth - orderConfig.takingAmount);
  });
