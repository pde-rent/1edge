...
describe("📊 Range Amount Calculator Strategies", function () {
  ...
  it("📊 Execute multi-token range calculation strategy", async function () {
    ...
    expect(finalBalances.makerDai).to.equal(
      initialBalances.makerDai - firstLegAmount,
    );
    expect(finalBalances.makerUsdc).to.equal(
      initialBalances.makerUsdc - secondLegAmount,
    );
    expect(finalBalances.makerWeth).to.equal(
      initialBalances.makerWeth + expectedWethAmount,
    );

    // Taker should have: +DAI, +USDC, -WETH
    expect(finalBalances.takerDai).to.equal(
      initialBalances.takerDai + firstLegAmount,
    );
    expect(finalBalances.takerUsdc).to.equal(
      initialBalances.takerUsdc + secondLegAmount,
    );
    expect(finalBalances.takerWeth).to.equal(
      initialBalances.takerWeth - expectedWethAmount,
    );
  });
