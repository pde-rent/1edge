...
describe("🔄 Pre-Interaction Approval Workflow", function () {
  ...
  async function deployPreInteractionSystem(): Promise<PreInteractionTestFixture> {
    ...
    const ApprovalPreInteraction = await ethers.getContractFactory(
    ...
    await weth
      .connect(taker)
      .transfer(await approvalPreInteraction.getAddress(), wethAmount);
  ...
  async function captureBalances(
    ...
    return {
      contractWeth: await tokens.weth.balanceOf(
        contracts.approvalPreInteraction.address,
      ),
      contractDai: await tokens.dai.balanceOf(
        contracts.approvalPreInteraction.address,
      ),
  ...
  it("🚀 Execute order with automatic token approval", async function () {
    const { tokens, contracts, chainId, accounts } =
      await deployPreInteractionSystem();
    ...
    const { compactSerialized: compactSignature } =
    ...
    expect(balancesAfter.contractWeth).to.equal(
      balancesBefore.contractWeth - order.makingAmount,
    );
    expect(balancesAfter.contractDai).to.equal(
      balancesBefore.contractDai + order.takingAmount,
    );

    // Taker should have gained WETH and lost DAI
    expect(balancesAfter.takerWeth).to.equal(
      balancesBefore.takerWeth + order.makingAmount,
    );
    expect(balancesAfter.takerDai).to.equal(
      balancesBefore.takerDai - order.takingAmount,
    );
