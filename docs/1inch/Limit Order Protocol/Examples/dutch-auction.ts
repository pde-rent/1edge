...
describe('🔥 Dutch Auction Orders', function () {
  ...
  it('🎯 Fill with making amount at 50% auction progress', async function () {
    ...
    // 🚀 Execute the fill
    await contracts.swap.connect(taker).fillOrderArgs(
      order,
      r,
      vs,
      makingAmount,
    ...
    expect(await tokens.dai.balanceOf(maker.address))
      .to.equal(balances.makerDaiBefore - makingAmount);
    expect(await tokens.dai.balanceOf(taker.address))
      .to.equal(balances.takerDaiBefore + makingAmount);
  ...
  it('🎯 Fill with taking amount at 50% auction progress', async function () {
    ...
    // ✅ Verify transfers
    expect(await tokens.dai.balanceOf(maker.address))
      .to.equal(balances.makerDaiBefore - expectedMakingAmount);
    expect(await tokens.dai.balanceOf(taker.address))
      .to.equal(balances.takerDaiBefore + expectedMakingAmount);
  ...
  it('🎯 Fill at auction start (maximum price)', async function () {
    const { tokens, contracts, order, signature, balances } = 
      await loadFixture(deployAndBuildDutchAuctionOrder);
